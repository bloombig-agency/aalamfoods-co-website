    ...
    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          name: formData.name,
          whatsapp: `https://wa.me/91${formData.whatsapp
            .replace(/\D/g, "")
            .slice(-10)}`,
          ...
          {status === "success" ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">
                We've received your order. We'll be in touch on WhatsApp
                shortly.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full bg-brand-primary text-white py-4 rounded-xl font-bold text-lg hover:bg-[#b35b30] transition-all transform hover:scale-[1.01] shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                >
                  {status === "loading" ? (
                    <>
